...
// PROTECTION : Demander confirmation si la DB existe
if (fs.existsSync(dbPath)) {
  console.log('⚠️  ATTENTION : Une base de données existe déjà !');
  console.log('🗂️  Toutes les données personnalisées (catégories créées, questions, etc.) seront DÉFINITIVEMENT PERDUES !');
  console.log('');
  ...
  rl.question('❓ Êtes-vous VRAIMENT sûr de vouloir continuer ? (tapez "SUPPRIMER" pour confirmer) : ', (answer) => {
    rl.close();
    
    if (answer.trim() !== 'SUPPRIMER') {
      console.log('✅ Opération annulée. Vos données sont sauves !');
      console.log('💡 Utilisez plutôt le script update-database.js pour les mises à jour non destructives.');
      process.exit(0);
    } else {
      console.log('⚠️  Vous avez confirmé. Suppression en cours...');
      proceedWithRebuild();
    }
  });
} else {
  // Pas de DB existante, on peut continuer
  proceedWithRebuild();
}

function proceedWithRebuild() {
